...
test('single HTML is offline and scripts parse',()=>{assert.ok(!/<script[^>]+src=/.test(html));assert.ok(!/(?:src|href)=["']https?:/.test(html));for(const [,js] of html.matchAll(/<script[^>]*>([\s\S]*?)<\/script>/g))new vm.Script(js);});
test('full-circle aim, spring strokes and gyro conserve linear/angular momentum in free space',()=>{
 const s=F.create(40,-200),p0=F.momentum(s),c0=F.com(s);
 run(s,20,t=>({aim:t*1.3,gyro:Math.sin(t*1.8),brake:t%3>2.6,kick:t%1<.35,tuck:t%1>.7}));finite(s);const p=F.momentum(s),c=F.com(s);
 near(p.x,p0.x);near(p.y,p0.y);near(p.angular,p0.angular,.001);near(c.x,c0.x);near(c.y,c0.y);
});
test('uniform gravity gives a ballistic COM despite aiming, tucking and gyro input',()=>{
 const s=F.create(120,200);s.b.vx=s.f.vx=145;s.b.vy=s.f.vy=-170;const c=F.com(s),n=720,t=n*F.DT;
 run(s,t,u=>({aim:u*5,kick:u%1<.4,tuck:u%1>.6,gyro:Math.sin(u*3)}),{...empty,gravity:F.P.gravity});
 const d=F.com(s);near(d.x,c.x+c.vx*t,1e-6);near(d.y,c.y+c.vy*t+.5*F.P.gravity*(t*t+t*F.DT),1e-5);near(d.vy,c.vy+F.P.gravity*t,1e-7);
});
...
test('right-click tuck takes priority over extension',()=>{const s=F.create();run(s,.4,{aim:Math.PI/2,kick:true,tuck:true});near(s.rest,F.P.tuck);});
test('the central leg has no direct radial torque',()=>{const s=F.create();run(s,.8,{aim:Math.PI/2,kick:true});near(s.b.w,0,1e-8);near(s.rotor.w,0,1e-8);});
test('aiming the massive foot gives equal and opposite chassis reaction',()=>{const s=F.create();run(s,.25,{aim:0});assert.ok(Math.abs(s.b.w)>.2);near(F.momentum(s).angular,0,.001);});
test('hook and rod have collision geometry, not only artwork',()=>{
 const s=F.create(0,0);const hook=F.contactList(s,{solids:[],pegs:[{x:47,y:-42,r:5}]});assert.ok(hook.some(c=>c.hookPart));
 const rod=F.contactList(s,{solids:[F.box(-10,48,20,4)],pegs:[]});assert.ok(rod.some(c=>c.type==='leg'));
});
test('long mixed-input contact simulation remains finite',()=>{const s=F.create(180,454);run(s,40,t=>({aim:Math.PI/2+Math.sin(t*.8)*1.2,gyro:Math.sin(t*2),kick:t%1.7<.3,tuck:t%1.7>1.4}),ground);finite(s);});
test('collision normals point outside a convex platform',()=>{const p=F.box(0,0,100,100);const top=F.circlePoly(50,-5,10,p);near(top.ny,-1);near(top.depth,5);const inside=F.circlePoly(50,4,10,p);near(inside.ny,-1);near(inside.depth,14);assert.equal(F.circlePoly(150,150,5,p),null);});

test('the hook supports the whole machine on a brass rail without a grab joint',()=>{
 const s=F.create(-5,36);const world={solids:[],pegs:[{x:0,y:0,r:9}]};
 run(s,6,{aim:Math.PI/2},world);finite(s);assert.ok(s.hookContacts>0);assert.ok(s.b.y>35&&s.b.y<65);assert.ok(Math.abs(s.b.x)<15);assert.ok(s.f.y>130);
});
test('a real protruding ledge carries a hanging machine',()=>{
 const world={solids:[F.box(0,0,500,1000),F.box(-65,-8,80,8,'brass')],pegs:[{x:-65,y:-4,r:8}]};
 const s=F.create(-70,32);run(s,6,{aim:Math.PI/2},world);finite(s);assert.ok(s.hookContacts>0);assert.ok(s.b.x< -30);assert.ok(s.b.y>15&&s.b.y<65);assert.ok(Math.hypot(s.b.vx,s.b.vy)<8);
});
test('turning the hook can physically release a rail',()=>{
 const s=F.create(-5,36),world={solids:[],pegs:[{x:0,y:0,r:9}]};run(s,2,{aim:Math.PI/2},world);
 run(s,4,{aim:Math.PI/2,gyro:-1},world);assert.ok(s.b.y>200,`height after release ${s.b.y}`);
});

test('a moving, initially separate machine can thread and catch the rail',()=>{
 const s=F.create(-70,0),world={solids:[],pegs:[{x:0,y:0,r:9}]};s.b.vx=s.f.vx=120;s.b.vy=s.f.vy=-120;
 assert.equal(F.contactList(s,world).length,0);run(s,6,{aim:Math.PI/2},world);
 assert.ok(s.hookContacts>0);assert.ok(s.b.y>0&&s.b.y<100);
});
...
vm.runInContext('const F=SpringheelPhysics;const {box,polygon}=F;'+html.slice(html.indexOf('function course('),html.indexOf('let state,level,index='))+';globalThis.courses=courses;',scope);
for(const index of [0,2,4,6])test(`course ${index+1}: repeated physical strokes can reach the exit`,()=>{
 const level=scope.courses[index],s=F.create(...level.start);let reached=false;
 for(let i=0;i<240*22;i++){
  const phase=(i/240)%1.1;
  F.step(s,{aim:2.08,kick:phase>.28&&phase<.55,tuck:phase>.92},level);
  if(Math.abs(s.b.x-level.goal.x)<45&&s.b.y<level.goal.y+10&&s.b.y>level.goal.y-175){reached=true;break;}
 }
